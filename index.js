...
function calculateTotal(cart,req) { 
    total = 0;
    for (let i=0; i<cart.length; i++){
        //if wee're offering a discounted price
...
//get takes 2 parameters .1.url 2 fn inside fn 2paramters  --req and res 
    ...
    var con = mysql.createConnection({
     ...
     con.connect((err)=> {
        if(err){
          ...
          //the values will be replaced from values to question mark
          con.query(query, [values], (err,result) => {
            for(let i = 0; i < cart.length; i++){
                var query= "INSERT INTO orders_items (order_id, product_id, product_name, product_price, product_image) VALUES  ?";
                var values = [ 
    ...
    con.connect((err)=> {
        if(err){
            ...
            con.query(query, [values],(err, result)=>{
                ...
                res.redirect('/thank_you', {order_id: order_id, transaction_id: transaction_id})
             })
          }
     })
